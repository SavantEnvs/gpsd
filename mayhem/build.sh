...
# $SANITIZER_FLAGS is folded into CC/CXX below and used to compile BOTH the scons-built gpsd
# library (libgps_static.a / libgpsd.a) and every harness .o — without this flag those TUs get
# ASan+UBSan but no SanCov edge counters, so all five targets see 0 edges despite fuzzing/crashing
# successfully. -fsanitize=fuzzer-no-link adds instrumentation only (no libFuzzer main), so it's
# safe on every TU including the ones linked into the *-standalone reproducers.
...
# ── 4) gpsd's own unit / known-answer tests, compiled HERE and only RUN by mayhem/test.sh (SPEC §6.3).
#       rlenv runs test.sh as the unprivileged runner, which must not rebuild the project under test.
#       Build them with NORMAL flags (env scrubbed of the sanitizer CC/flags exported above) so the
#       oracle stays independent of the sanitized fuzz build. Fuzz targets are already linked into
#       $OUT, so re-flagging the shared variant-dir objects here does not affect them. ─────────────────
TESTS="test_bits test_crc test_json test_packet test_mktime test_geoid test_matrix test_timespec test_trig test_libgps"
TGTS=""; for t in $TESTS; do TGTS="$TGTS ${BUILDDIR#"$SRC"/}/tests/$t"; done
echo "=== building gpsd unit tests (normal flags) ==="
# shellcheck disable=SC2086  # TGTS is a word list of scons targets
